...
if db.query(models.Project).count() == 0:
    ...
    db.add_all(projects)
    db.commit()
    print("案件サンプルデータを投入しました")

    # 活動履歴（案件投入後にIDが確定してから追加）
    projects_db = db.query(models.Project).all()
    id_map = {p.business_number: p.id for p in projects_db}
    logs = [
        models.ActivityLog(
            project_id=id_map.get("R6-001"),
    ...
    db.add_all([l for l in logs if l.project_id])
    db.commit()
    print("活動履歴サンプルデータを投入しました")
